...
use crate::{
	implementations::fill_description::{encode_fill_description, extract_verified_fill_from_logs},
	utils::{
...
use solver_types::{
	...
	Order, PusherL2Params, Schema, Transaction, TransactionHash, TransactionReceipt,
	TransactionType,
};
...
impl BroadcasterSettlement {
	...
	async fn build_submission_from_post_fill_receipt(
		...
		let fill_logs = self
			...
			.await?;
		let fill_payload =
			self.parse_fill_payload_from_logs(order, &fill_logs, destination_chain)?;
	...
	fn parse_fill_payload_from_logs(
		...
		let verified_fill =
			extract_verified_fill_from_logs(logs, order, order_id_bytes, destination_chain)?;
...
impl SettlementInterface for BroadcasterSettlement {
	...
	async fn generate_post_fill_transaction(
		...
		let fill_payload =
			self.parse_fill_payload_from_logs(order, &fill_receipt.logs, destination_chain)?;
		let payloads = vec![fill_payload];
...
mod tests {
	...
	fn build_test_order_for_emitter_tests(
		...
		// Verified field list (crates/solver-types/src/standards/eip7683.rs:112-):
		//   user, nonce, origin_chain_id, expires, fill_deadline, input_oracle,
		//   inputs, order_id, gas_limit_overrides, outputs, raw_order_data,
		//   signature, sponsor, lock_type
		// NOTE: there is NO `settlement_name` field on Eip7683OrderData; that field
		// lives on `Order`, not `Eip7683OrderData`.
		...
		// Settler in the Order's output_chains: the 20-byte address derived from the
		// signed bytes32 settler. The helper's job is to verify this matches.
	...
	fn encode_output_filled_data(
		...
		// SolEvent::encode_data returns the non-indexed data portion.
		event.encode_data()
	}
	// === End shared helpers ===

	#[test]
	fn broadcaster_payload_includes_output_context() {
		use crate::implementations::fill_description::{
			encode_fill_description as encode_shared_fill_description, VerifiedFill,
		};
		...
		token[12..32].copy_from_slice(&[0x22; 20]);
		let mut recipient = [0u8; 32];
		recipient[12..32].copy_from_slice(&[0x33; 20]);

		let mut output =
			make_mandate_output([0x11; 32], settler_bytes32, 137, token, amount, recipient);
		...
		let expected_payload = encode_shared_fill_description(&verified_fill, order_id).unwrap();
		let omitted_context_payload = encode_shared_fill_description(
			&VerifiedFill {
				output: make_mandate_output(
					[0x11; 32],
					settler_bytes32,
					137,
					token,
		...
		let actual_payload = settlement
			.parse_fill_payload_from_logs(&order, &[log], 137)
			.unwrap();

		assert_eq!(actual_payload, expected_payload);
		assert_ne!(
			expected_payload, omitted_context_payload,
			"test setup must make non-empty context change the encoded payload"
		);
		assert!(
			actual_payload.ends_with(&[0, 1, 0]),
			"Broadcaster fill payload must end with context length 1 and byte 0x00"
		);
	}
	...
	fn test_extract_fill_details_rejects_mismatched_mandate_output() {
		...
		// Log emitted from the correct settler but with TAMPERED amount inside MandateOutput.
		let tampered_output = make_mandate_output(
			[0x11; 32],
			settler_bytes32,
			137,
			[0x22; 32],
			alloy_primitives::U256::from(9999u64), // wrong
			[0x33; 32],
		);
		let log_data = encode_output_filled_data(
			order_id,
	...
	fn test_encode_fill_description_rejects_oversized_call_data() {
		...
		output.call = vec![0u8; (u16::MAX as usize) + 1];
		let fill = crate::implementations::fill_description::VerifiedFill {
			solver_identifier: [0x11; 32],
			timestamp: 1,
			output,
		};

		let err = encode_fill_description(&fill, [0x22; 32]).unwrap_err();
	...
	#[test]
	fn test_extract_verified_fill_rejects_missing_outputs() {
		let order = OrderBuilder::new()
		...
		let err = match extract_verified_fill_from_logs(&[], &order, [0u8; 32], 137) {
			Ok(_) => panic!("missing outputs should error"),
			Err(err) => err,
		};
		assert!(err.to_string().contains("Order has no output"));
	}
	...
	async fn test_generate_post_fill_transaction_builds_submit_call() {
		...
		let input_oracle = solver_types::Address(vec![0x33; 20]);
		let output_oracle = solver_types::Address(vec![0x44; 20]);
		// The OutputFilled-emitter check requires the order's MandateOutput.settler
		// to match the log's emitter address. Use a dedicated output-settler addr
		// for both ends so the new helper accepts the log.
		let output_settler_addr = solver_types::Address(vec![0x55; 20]);
		let output_settler_b32 = address_to_bytes32(&output_settler_addr);
		let mandate_output = MandateOutputBuilder::new()
			.oracle(address_to_bytes32(&output_oracle))
			.settler(output_settler_b32)
		...
		// Build a properly ABI-encoded OutputFilled log emitted from the order's
		// settler so the new helper's emitter+payload checks pass.
		let order_id_bytes = order_id_to_bytes32(&order.id).unwrap();
		let event_data = encode_output_filled_data(
			order_id_bytes,
			[0x11; 32],
			123u32,
			&mandate_output,
		...
		let receipt = TransactionReceipt {
			...
			logs: vec![solver_types::Log {
				address: output_settler_addr.clone(),
				topics: vec![
					solver_types::H256(
						<solver_types::standards::eip7683::interfaces::OutputFilled
							as alloy_sol_types::SolEvent>::SIGNATURE_HASH.0,
					),
					solver_types::H256(order_id_bytes),
				],
				data: event_data,
				..Default::default()
			}],
